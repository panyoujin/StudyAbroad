#获取用户列表
select_user_list="SELECT u.`Id`,u.`Account`,u.`Phone`,u.`Password`,u.`UserType`,ui.`Name`,ui.`RealName`,u.`Phone` ,u.`CreateTime` "\
...
#获取用户详情
select_user_info="SELECT u.`Id`,u.`Account`,u.`Phone`,u.`Password`,u.`UserType`,ui.`Name`,ui.`RealName`,u.`Phone`,ui.`Sex`,ui.`Age`,ui.`Email`,ui.`Education`,ui.`Address`,ui.`HeadImage`,ui.`IDCard`,ui.`IDCardJust`,ui.`IDCardBack` "\
                      ",(SELECT GROUP_CONCAT(sa.`Name`) FROM `Base_ServiceArea` sa WHERE FIND_IN_SET(sa.`Id`,ui.`ServiceAreaId`)>0) AS AreaName" \
                      ",(SELECT GROUP_CONCAT(st.`Name`) FROM `Base_ServiceType` st WHERE FIND_IN_SET(st.`Id`,ui.`ServiceTypeId`)>0) AS TypeName " \
                    "FROM `U_User` u "\
                    "LEFT JOIN `U_UserInfo` ui ON u.`Id`=ui.`UserId` "\
...
insert_user="INSERT INTO `U_User` (`Id`,`Account`,`Phone`,`Password`,`UserType`,`CreateUserID`,`CreateTime`,`ModifUserID`,`ModifTime`,`IsDelete`)  " \
            ...
            "INSERT INTO `U_UserInfo` (`UserId`,`Name`,`RealName`,`Sex`,`Age`,`Education`,`Address`,`Email`,`HeadImage`,IDCard,IDCardJust,IDCardBack,`CreateUserID`,`CreateTime`,`ModifUserID`,`ModifTime`,`IsDelete`)  " \
                "VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',NOW(),'%s',NOW(),FALSE) ;"

...
#通过帐号查询用户信息
select_user_info = "SELECT `Id`,`Account`,`Phone`,`UserType`,`LoginTime`,`LoginToken`,`LoginIP` FROM `U_User` WHERE Account ='%s'"
#通过token查询用户信息
select_user_login_info = "SELECT `Id`,`Account`,`Phone`,`UserType`,`LoginTime`,`LoginToken`,`LoginIP` FROM `U_User` WHERE LoginToken ='%s'"
#修改Token
